...
(function($) {
    $.fn.fullpage = function(options) {
        ...
        $('.section').each(function(index){
            ...
            // if($(window).innerWidth() > 787){hanvika
        ...
        */
                ...
                if (!isMoving) { //if theres any #
                    ...
                    if(activeSection.find('.slides').length && Math.abs(touchStartX - touchEndX) > (Math.abs(touchStartY - touchEndY) + xThreshold)){
                        if(touchStartX > touchEndX){
                            // activeSection.find('.controlArrow.next').trigger('click');hanvika
                        }
                        else if(touchStartX < touchEndX){
                            // activeSection.find('.controlArrow.prev').trigger('click');hanvika
                        }
                    }
                    //vertical scrolling
                    else{
                        ...
                        if(touchStartY > touchEndY){
                            ...
                            if(scrollable.length > 0){//hanvika
        ...
        function scrollPage(element, callback) {
            ...
            element.addClass('active section-ani');
            setTimeout(function(){
                element.siblings().removeClass('active')
            },200)
            // alert('6--');//hanvika

            var liSelected = element.find('.bullets li[data-focus]').attr('data-focus');
            
            
            setTimeout(function(){
                element.find('.bullets li[data-focus]').addClass(liSelected);
                if($('.logo-visible').hasClass('active')){
                    $('.header').addClass('showLogo');
                }
                if($('.no-logo').hasClass('active')){
                    $('.header').removeClass('showLogo');
                }
                if($('.ba-hide').hasClass('active')){
                    $('.backToTop').removeClass('showArrow');    
                }
                if($('.ba-show').hasClass('active')){
                    $('.backToTop').addClass('showArrow');    
                }
        ...
        // $('.menu-content-wrap .nav li span').on('click',function(){
                var clickId = $(this).attr('data-href'),
                    clickIndex = $('.section#'+clickId).index() - 1;
                $('.menu-button').removeClass('close');
                $('.menu-overlay').hide();
                $('.header').removeClass('header-trans');
                $('#fullPage-nav a').eq(clickIndex).trigger('click');
            });
...
function sliderWidth(){
    ...
    $('.feedback-slider-wrap').css('width',winWidth);

    var total = 0,
    winw = $(window).width();
    $('.mmv-nv').parent('div').addClass('mmv-scrl');
    $('.feedback-slider li').each(function(){
        var test =total += $(this).outerWidth() + 20;//+ 20;
    });
    $('.feedback-slider').css('width',total);
}
...
$(document).ready(function() {
    ...
    if($(window).innerWidth() < 767){
       ...
       $('.banner-content').height(WinHeight - $('.video-wrap').outerHeight()  - 30);

    }
    hamburgerClick();
    $('.no-logo').addClass('screen-1');
